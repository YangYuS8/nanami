...
#[test]
fn permission_requested_serializes_full_json_shape() {
    ...
    assert_eq!(json["target"], "/home/user/Code/nanami");
    assert_eq!(json["reason"], "Need to read project files for analysis");
    assert_eq!(json["scope"], "task");
    assert_eq!(json["expires"], "task_completed");
}

#[test]
fn permission_resolved_allow_once_serializes_full_json_shape() {
    let event = EventEnvelope::new(
        "evt_perm_res_001",
        timestamp(),
...
#[test]
fn permission_resolved_allow_for_task_serializes_full_json_shape() {
    let event = EventEnvelope::new(
        "evt_perm_res_002",
        timestamp(),
        Event::PermissionResolved(PermissionResolvedPayload {
...
#[test]
fn permission_resolved_deny_serializes_full_json_shape() {
    let event = EventEnvelope::new(
        "evt_perm_res_003",
        timestamp(),
        Event::PermissionResolved(PermissionResolvedPayload {
